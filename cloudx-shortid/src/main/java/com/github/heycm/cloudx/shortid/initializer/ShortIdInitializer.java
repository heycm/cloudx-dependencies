package com.github.heycm.cloudx.shortid.initializer;
...
public class ShortIdInitializer {
    ...
    public void initialize() {
        String findTable = "SHOW TABLES LIKE 'cloudx_short_id'";
        Integer i = jdbcTemplate.queryForObject(findTable, Integer.class);
        if (i != null) {
            LOGGER.info("Table 'cloudx_short_id' already exists.");
            return;
        }
        String createTable = """
                CREATE TABLE `cloudx_short_id` (
                  ...
                  PRIMARY KEY (`id_key`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='CloudX-短ID生成表'
                """;
        jdbcTemplate.update(createTable);
        LOGGER.info("Table 'cloudx_short_id' created successfully.");
    }
}
