package com.github.heycm.cloudx.shortid.service;

import com.github.heycm.cloudx.shortid.entity.ShortId;
import com.github.heycm.cloudx.shortid.exception.ShortIdException;
...
public class ShortIdService {
    ...
    private int incrShortId(ShortId shortId) {
        String sql = "UPDATE cloudx_short_id SET id_value = id_value + id_step WHERE id_key = ? AND id_value = ?";
        return jdbcTemplate.update(sql, shortId.getIdKey(), shortId.getIdValue());
    }

    private ShortId findIdKey(String idKey) {
        String sql = "SELECT id_key, id_value, id_step FROM cloudx_short_id WHERE id_key = ?";
        try {
